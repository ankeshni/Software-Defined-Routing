...
uint16_t my_router_port(struct initpayload* init_payload)
    {
        for(int i=0;i<init_payload->total_routers;i++)
    {
        if(init_payload->router_list[i].cost==0)
        {
            return init_payload->router_list[i].router_port;
        }
        
    }
    
        perror("Unable to find my router port");
    
    }
uint16_t my_data_port(struct initpayload* init_payload)
    {
        for(int i=0;i<init_payload->total_routers;i++)
    {
        if(init_payload->router_list[i].cost==0)
        {
            return init_payload->router_list[i].data_port;
        }
        
    }
    
        perror("Unable to find my data port");
...
int create_router_sock(struct initpayload* init_payload) //UDP
{
    int sock;
    ...
    router_addr.sin_port = htons(my_router_port(init_payload));
    
    if(bind(sock, (struct sockaddr *)&router_addr, sizeof(router_addr)) < 0)
        ERROR("bind() failed");
printf("router socket");
//    if(listen(sock, 5) < 0)
//        ERROR("listen() failed"); //UDP port doesnt listen

    // LIST_INIT(&control_conn_list);

    return sock;   
}
int create_data_sock(struct initpayload* init_payload)
{
    int sock;
    ...
    data_addr.sin_port = htons(my_data_port(init_payload));
...
bool control_recv_hook(int sock_index)
{
 ...
 void update_response(int sock_index, char *cntrl_payload)
	...
	free(cntrl_response);
        ...
        for(int i=0;i<5;i++)
        {
            if((*((uint16_t*)(cntrl_payload+0)))==*((uint16_t*)(&(routing_table->info[i]))+0))
            {
                *(((uint16_t*)(&(routing_table->info[i]))+3))=  (uint16_t)ntohs(ntohs(*((uint16_t*)cntrl_payload+1)));
            }   
        }
...
void init_response(int sock_index, char *cntrl_payload)
 ...
 void init_router_ports()
 {
      router_socket = create_router_sock(init_payload);
      FD_SET(router_socket, &master_list);
      head_fd = router_socket;
      data_socket = create_data_sock(init_payload);
 ...
 void save_init_payload(char *cntrl_payload)
 {
      ...
      uint16_t total_routers=ntohs(*(uint16_t*) cntrl_payload);
       update_interval=ntohs(*(((uint16_t*) cntrl_payload)+1));
 ...
 {
     // routing_table =(struct CTRL_TABLE_RESPONSE*) malloc(sizeof(struct CTRL_TABLE_RESPONSE));
	...
	/////cntrl_response_payload =(char *) malloc(10*sizeof(uint32_t));
      ...
      // struct CTRL_TABLE_RESPONSE *routing_table=
  ...
  {routing_table =(struct CTRL_TABLE_RESPONSE*) malloc(sizeof(struct CTRL_TABLE_RESPONSE));
    ...
    * next hop      cost    */
    ...
    {   //remove starting padding of 0x0000
        if(init_payload->router_list[i].cost==0)
        {
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(init_payload->router_list[i].id);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(0);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(init_payload->router_list[i].id);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(0);
            //u_16=8;//skip padding  
        }
        else if(init_payload->router_list[i].cost==INF)
        {
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(init_payload->router_list[i].id);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(0);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(INF);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(INF);
            //u_16=8;//skip padding
        }
        else 
        {
            ...
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(0);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(init_payload->router_list[i].id);
            *(((uint16_t*)(&(routing_table->info[i]))+u_16++))= htons(init_payload->router_list[i].cost);
 ...
 void make_neighbour_list(struct initpayload* init_payload)
 { 
   for(int i=0;i<init_payload->total_routers;i++)
   {
       if((init_payload->router_list[i].cost!=0)&&(init_payload->router_list[i].cost!=INF))
       {
           neighbour_list[tot_neighbours]= (struct NEIGHBOUR_list*)malloc(sizeof(struct NEIGHBOUR_list*));
           
           neighbour_list[tot_neighbours]->id =init_payload->router_list[i].id;
           neighbour_list[tot_neighbours]->r_port =init_payload->router_list[i].router_port;
           neighbour_list[tot_neighbours]->d_port =init_payload->router_list[i].data_port;
           neighbour_list[tot_neighbours]->ip =init_payload->router_list[i].ip;
           neighbour_list[tot_neighbours]->update_rcv_flag =1;
           neighbour_list[tot_neighbours]->cost =init_payload->router_list[i].cost;
           neighbour_list[tot_neighbours++]->link_active =1;
